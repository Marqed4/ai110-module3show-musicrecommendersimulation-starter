...
There's also a subtler bias in how the "energy gap" plays out against this specific catalog: `energy_diff = abs(song["energy"] - user_energy)` is symmetric in the math (too calm and too hyped are penalized equally per unit of distance), but the catalog's actual songs only span energy `0.28` to `0.93`, clustered mostly in the `0.7`-`0.9` range. A user who asks for very low energy (near `0.0`) is structurally underserved: the closest available song is still `0.28` away in the best case, a gap the energy score can never fully close, no matter how well genre and mood match. A user asking for very high energy (near `1.0`, as tested in "Max Energy Extreme") fares much better, since the closest song is only `0.07` away. The scoring rule itself doesn't play favorites, but it inherits whatever imbalance already exists in the dataset's energy distribution, and low-energy listeners currently get the worse end of that imbalance.
